...
import { authenticateToken, requirePhoneVerification } from '@/middleware/authMiddleware';
const router = Router();
...
router.post('/', [
    ...
    try {
        ...
        const { product_name, brand, category, store_name, store_type, purchase_price, purchase_date, submission_location, raw_submission_data } = req.body;
        const db = getDatabase();
        // Check for potential duplicates
        const duplicateCheck = await db.query(`SELECT id FROM npd_submissions 
       WHERE user_id = $1 
       AND submitted_product_name ILIKE $2 
       AND submitted_brand ILIKE $3 
       AND created_at > NOW() - INTERVAL '24 hours'`, [req.user.id, product_name, brand]);
        ...
        // Create submission
        const result = await db.query(`INSERT INTO npd_submissions (
      ...
      RETURNING id, status, base_reward, created_at`, [
        ...
        await db.query('UPDATE users SET total_submissions = total_submissions + 1 WHERE id = $1', [req.user.id]);
        res.status(201).json({
            success: true,
            data: {
                submission: {
...
router.get('/:id', [
    ...
    try {
      ...
      WHERE s.id = $1 AND s.user_id = $2`, [id, req.user.id]);
        ...
        const imagesResult = await db.query('SELECT id, image_url, image_type, quality_score FROM product_images WHERE submission_id = $1', [id]);
        const submission = result.rows[0];
...
router.post('/:id/images', [
    ...
    try {
        ...
        const submissionCheck = await db.query('SELECT id FROM npd_submissions WHERE id = $1 AND user_id = $2', [id, req.user.id]);
        ...
        // Add image
        const result = await db.query(`INSERT INTO product_images (submission_id, image_url, image_type, file_size, dimensions, uploaded_by)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING id, image_url, image_type, created_at`, [id, image_url, image_type, file_size || null, dimensions || null, req.user.id]);
        res.status(201).json({
            success: true,
