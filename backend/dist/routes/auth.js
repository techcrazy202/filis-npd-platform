...
router.post('/register', [
    ...
    try {
        ...
        // Create user
        const result = await db.query(`INSERT INTO users (email, phone, password_hash, full_name) 
       VALUES ($1, $2, $3, $4) 
       RETURNING id, email, phone, full_name, user_tier, created_at`, [email, phone, password_hash, full_name]);
        const user = result.rows[0];
        // Generate JWT token
        const token = jwt.sign({ userId: user.id, email: user.email }, env.JWT_SECRET, { expiresIn: '24h' });
        res.status(201).json({
            success: true,
            data: {
                user: {
...
router.post('/login', [
    ...
    try {
        ...
        const db = getDatabase();
        // Find user
        const result = await db.query('SELECT id, email, phone, password_hash, full_name, user_tier, email_verified, phone_verified, is_active FROM users WHERE email = $1', [email]);
...
router.post('/send-otp', [
    ...
    try {
        ...
        await db.query(`INSERT INTO otp_codes (phone, code, purpose, expires_at) 
       VALUES ($1, $2, $3, $4) 
       ON CONFLICT (phone, purpose) 
       DO UPDATE SET code = $2, expires_at = $4, attempts = 0, created_at = NOW()`, [phone, code, purpose, expires_at]);
...
router.post('/verify-otp', [
    ...
    try {
        ...
        const db = getDatabase();
        // Find OTP
        const result = await db.query('SELECT * FROM otp_codes WHERE phone = $1 AND purpose = $2 AND expires_at > NOW()', [phone, purpose]);
        if (result.rows.length === 0) {
            return res.status(400).json({
                success: false,
...
router.get('/me', authenticateToken, async (req, res) => {
    try {
        const db = getDatabase();
        const result = await db.query(`SELECT id, email, phone, full_name, user_tier, total_earnings, 
