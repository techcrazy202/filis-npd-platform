...
class DataMigrationService {
      ...
      try {
        ...
        if (processed) {
          ...
          params.push(
            ...
            processed.industry,
            processed.sector,
            processed.sub_sector,
            processed.segment,
            processed.sub_segment,
            processed.category,
            processed.continents,
            processed.country,
            processed.geo,
            processed.country_of_origin,
            processed.ingredients_list,
            processed.calories,
            processed.claims,
            processed.flavour,
            processed.price,
            processed.volume_scale,
            processed.volume_subscale,
            processed.currency,
            processed.company_name,
            processed.standardized_company_name,
            processed.product_link,
            processed.source_name,
            processed.website_address
          );
          paramIndex += 27;
        }
      } catch (error) {
  ...
  private transformRecord(raw: RawProductData): any | null {
  ...
  private async generateMigrationReport(): Promise<void> {
    ...
    if (report.topCategories.length > 0) {
      console.log('\n🏷️  Top Categories:');
      report.topCategories.forEach((cat: any, index: number) => {
    ...
    if (report.topCountries.length > 0) {
      console.log('\n🌍 Top Countries:');
      report.topCountries.forEach((country: any, index: number) => {
